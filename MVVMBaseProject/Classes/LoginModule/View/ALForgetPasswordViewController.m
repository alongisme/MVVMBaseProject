//
//  ALForgetPasswordViewController.m
//  MVVMBaseProject
//
...
#import "ALForgetPasswordViewController.h"
#import "ALForgetPasswordViewModel.h"

@interface ALForgetPasswordViewController ()
@property (nonatomic, strong) ALForgetPasswordViewModel *viewModel;
...
@implementation ALForgetPasswordViewController
...
- (void)bindViewModel {
    ...
    [[self.updateBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        @strongify(self);
        [[[[self.viewModel.updateCommand execute:nil] filter:^BOOL(NSNumber *value) {
            @strongify(self);
            if(value.integerValue == ALErrorPasswordNotEquel) {
                [self.view showHudInWindowError:@"两次密码不一致！"];
            } else {
                [self.view showHudInWindowError:@"密码重置成功！"];
                return YES;
            }
            return NO;
    ...
    [[[RACSignal merge:@[self.viewModel.getCodeCommand.executing,self.viewModel.updateCommand.executing]] doNext:^(id x) {
        @strongify(self);
...
- (UIButton *)updateBtn {
    if(!_updateBtn) {
        ...
        [_updateBtn setTitle:@"密码重置" forState:UIControlStateNormal];
        [_updateBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        _updateBtn.layer.masksToBounds = YES;
        _updateBtn.layer.borderColor = [UIColor blueColor].CGColor;
